...
# True Positive Rate Parity for Multiclass with handling for sparse groups
def equalize(data, metaclassifier, X_train, y_train, X_test, y_test, x_test_indices, x_train_indices, section_equalizer, xgboost, randomforest):
    ...
    for class_label in np.unique(y_train):
        ...
        # Apply ThresholdOptimizer for True Positive Rate Parity on groups with sufficient data
        try:
    ...
    print(f"Overall accuracy after True Positive Rate Parity post-processing: {overall_accuracy * 100.0}%")
